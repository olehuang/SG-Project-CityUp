...
# 指定测试照片的路径：本地路径里卢浮宫金字塔的那张照片
image_path = r"E:\anyFile\Louvre_test.jpg"

# Der offizielle Breiten- und Längengrad der Pyramide des Louvre卢浮宫金字塔的经纬度
...
if photo_gps is None:
    print("未能从图片中提取GPS信息，请检查图片EXIF。Failed to extract GPS information from image, please check image EXIF.")
else:
    # 2. 测试 check_photo_location
...
with open(image_path, "rb") as f:
    image_bytes = f.read()
print("_________________________________________________________")
is_clear, var = is_image_blurry(image_bytes, threshold=350)
